...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
	  <!--Длинная форма записи для создания узла-элемент  <xsl:element name="goods"> -->
    <goods>
      <!--Создание узла-комментария-->
      <xsl:comment> Всего элементов: <xsl:value-of select="count(items/element)"/> </xsl:comment>
      <!--Создание узлов имена которых совпадают с значением name-->
      <xsl:element name="{items/element[@name]}"/>
