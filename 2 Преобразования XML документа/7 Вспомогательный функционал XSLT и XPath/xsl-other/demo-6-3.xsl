...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				<h1>Наши занятия</h1>
				<xsl:apply-templates select="/demo/row" />
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template match="row">
		<p>
			...
			<xsl:text> </xsl:text>
			<xsl:value-of select="document('')/xsl:stylesheet/my:months/my:month[@no = substring(current()/@date, 6, 2)]" />
			<xsl:text> </xsl:text>
