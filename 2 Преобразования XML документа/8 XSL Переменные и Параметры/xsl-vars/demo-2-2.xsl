			...
			<body>
				<h1>Книги про XML</h1>
					<xsl:call-template name="textBlock">
						...
						<xsl:with-param name="text" select="concat(count($xmlBooks), ' книг.')" />
						<xsl:with-param name="background" select="'#ffd'" />
					</xsl:call-template>

					<xsl:call-template name="textBlock">
						<xsl:with-param name="title" select="'Средняя цена'" />
						<xsl:with-param name="text" select="concat(sum($xmlBooks/price) div count($xmlBooks), ' руб.')" />
